...
class Pinot(BaseSQLQueryRunner):
    ...
    def __init__(self, configuration):
        ...
        self._controller = f"{self.configuration['controller_host']}:{self.configuration['controller_port']}"
    ...
    def _get_tables(self, schema):
        ...
        for table_name in r.json()["tables"]:
            ...
            _get_table_schema(schema, table_name)

        return list(schema.values())

    def run_query(self, query, user):
        _broker = self.configuration["broker_host"]
        _port = self.configuration["broker_port"]

        connection = connect(
            host=self._broker, port=_port, path="/query/sql", scheme=self._proto
        )
